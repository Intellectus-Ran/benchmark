...
def plot_inter_arrival_times(data_dict, save_path):
    """Inter-arrival time 그래프 생성 (nonrt vs rt vs timer 비교)"""
    plt.figure(figsize=(16, 10))
    
    # 색상 정의 - Timer 버전 추가 (NonRT, RT 모두)
    colors = {
        'ROS2-NonRT': 'red',
        'ROS2-RT': 'orange',
        'FastDDS-NonRT': 'blue',
        'FastDDS-RT': 'skyblue',
        'FastDDS-Timer-NonRT': 'purple',     # Timer NonRT 버전
        'FastDDS-Timer-RT': 'hotpink',       # Timer RT 버전
        'Zenoh-NonRT': 'green',
    ...
    plt.title('Inter-arrival Times Comparison: NonRT vs RT vs Timer Across Middleware', fontsize=14)
...
def plot_messages_per_second(data_dict, save_path):
    """초당 메시지 수 그래프 생성 (nonrt vs rt vs timer 비교)"""
    plt.figure(figsize=(16, 10))
    ...
    plt.title('Messages per Second Comparison: NonRT vs RT vs Timer Across Middleware', fontsize=14)
    plt.legend(fontsize=10, loc='upper right')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
...
def main():
    # CSV 파일 경로 설정 (NonRT, RT, Timer 버전 - Timer는 NonRT/RT 모두)
    csv_files = {
        'FastDDS-NonRT': 'nonrt/fastdds_timestamp.csv',
        'FastDDS-RT': 'rt/fastdds_timestamp.csv',
        'FastDDS-Timer-NonRT': 'nonrt/fastdds_timer_timestamp.csv',  # Timer NonRT 버전
        'FastDDS-Timer-RT': 'rt/fastdds_timer_timestamp.csv',        # Timer RT 버전
    ...
    for middleware_version, csv_path in csv_files.items():
        ...
        if os.path.exists(full_path):
            ...
            if 'FastDDS' in middleware_version:
                if 'Timer' in middleware_version:
                    if 'NonRT' in middleware_version:
                        intervals = np.full(num_messages-1, 10)  # Timer NonRT: 일정한 간격
                        intervals += np.random.normal(0, 1.0, num_messages-1)  # 약간 더 큰 노이즈
                    else:  # Timer RT
                        intervals = np.full(num_messages-1, 10)  # Timer RT: 매우 일정한 간격
                        intervals += np.random.normal(0, 0.3, num_messages-1)  # 매우 작은 노이즈
                elif 'NonRT' in middleware_version:
    ...
    if data_dict:
        ...
        inter_arrival_path = os.path.join(base_dir, 'inter_arrival_times_with_timer.png')
        plot_inter_arrival_times(data_dict, inter_arrival_path)
        
        # Messages per second 그래프
        messages_per_sec_path = os.path.join(base_dir, 'messages_per_second_with_timer.png')
        plot_messages_per_second(data_dict, messages_per_sec_path)
        
        # 통계 정보 출력
        print("\n=== Statistics Summary (NonRT vs RT vs Timer) ===")
        for middleware_version, timestamps in data_dict.items():
            if timestamps is not None and len(timestamps) > 1:
                inter_arrivals = calculate_inter_arrival_times(timestamps)
                if len(inter_arrivals) > 0:
        ...
        for middleware in middleware_types:
            ...
            timer_nonrt_key = f"{middleware}-Timer-NonRT"
            timer_rt_key = f"{middleware}-Timer-RT"
            
            versions_data = {}
            for key in [nonrt_key, rt_key, timer_nonrt_key, timer_rt_key]:
                if key in data_dict and data_dict[key] is not None and len(data_dict[key]) > 1:
                    inter_arrivals = calculate_inter_arrival_times(data_dict[key])
                    if len(inter_arrivals) > 0:
                        versions_data[key] = {
                            'jitter': np.std(inter_arrivals),
                            'avg': np.mean(inter_arrivals)
                        }
            
            if versions_data:
                print(f"\n{middleware}:")
                for version, stats in versions_data.items():
                    version_name = version.replace(f"{middleware}-", "")
                    print(f"  {version_name} - Jitter: {stats['jitter']:.2f} ms, Avg: {stats['avg']:.2f} ms")
    else:
